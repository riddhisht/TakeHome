...
# profiling
profiles_df, cluster_profiles_dict = compute_profiles_from_processed(df_proc=X_proc_df,labels=labels,cat_cols=cat_cols,num_cols=num_cols,top_k_cat=TOP_K_CAT,top_k_num=TOP_K_NUM,out_dir=OUT_DIR)

# visualization
vis_df = pd.DataFrame({'pc1': X_pca[:, 0],'pc2': X_pca[:, 1],'cluster': labels})
plot_pca(vis_df, OUT_DIR)
unique_clusters = sorted(pd.unique(labels))
plot_cluster_profiles(profiles_df=profiles_df,clusters=unique_clusters,out_dir=OUT_DIR,top_k=10)
